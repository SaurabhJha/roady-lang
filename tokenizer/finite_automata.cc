...
void TransitionTableRow::add_input_state_pair(
    const std::string &input, int next_state) {
...
void TransitionTable::add_new_transition(
...
TransitionTable TransitionTable::combine_with(
    const TransitionTable& other_table) {
  TransitionTable combined_table;
  // Add all transitions from the first transition table
  // to the resultant combined table.
  for (auto& state_row_pair : table_) {
  ...
  // Add all transitions from the second transition table
  // to the resultant combined table.
...
void FiniteAutomata::add_new_transition(
    int prev_state, int next_state, const std::string &input) {
  auto row = table_[prev_state];
  row.add_input_state_pair(input, next_state);
...
FiniteAutomata FiniteAutomata::union_with(FiniteAutomata other_automata) {
  ...
  // Add new transitions to complete the union.
  // The new transitions are.
  // 1. The new start state has an epsilon transition to the
  //    start states of both operand automata.
  // 2. The final states of each operand automaton have an
  //    epsilon transition to the new final state.
  ...
  FiniteAutomata combined_automata(
      combined_start_state, combined_final_state, combined_table);
...
FiniteAutomata FiniteAutomata::concatenate_with(FiniteAutomata other_automata) {
  ...
  // Add a new transition to complete the concatenation.
  // The new transitions is an epsilon transition from the
  // final state of the first operand automaton to the start
  ...
  // Construct a new automata using the new transition table.
  // The start state is the start state of the first operand
  // automaton and the final state is the final state of the
  // second operand automaton.
  auto start_state1 = get_start_state();
...
void FiniteAutomata::move(const std::string& input) {
  ...
  for (auto current_state : current_states_) {
    ...
    for (auto immediate_next_state : immediate_next_states)
      for (auto closed_next_state :
        compute_epsilon_closure(immediate_next_state))
